...
assert.match(styleSource, /max-height: 94vh/);
assert.match(styleSource, /max-height: 98vh/);
assert.match(styleSource, /\.pf-scroll[\s\S]*flex: 1 1 auto/);
assert.match(styleSource, /\.pf-scroll[\s\S]*min-height: 0/);
...
assert.match(indexSource, /floatingButton: null/);
assert.match(indexSource, /if \(existing && existing === state\.overlay\) return/);
assert.match(indexSource, /if \(existing && existing === state\.settingsPanel\) return/);
assert.match(indexSource, /button && button !== state\.floatingButton/);
assert.match(indexSource, /staleOverlay && staleOverlay !== state\.overlay/);
assert.match(indexSource, /staleSettings && staleSettings !== state\.settingsPanel/);
assert.match(indexSource, /staleFloatingButton && staleFloatingButton !== state\.floatingButton/);
assert.match(indexSource, /面板已打开，但角色或世界书读取失败/);
assert.match(styleSource, /width: 65px;[\s\S]*width: var\(--pf-fab-size, 65px\)/);
assert.match(indexSource, /isPhoneViewport/);
assert.match(indexSource, /pf-open-button/);
assert.match(indexSource, /button\.style\.width = `\$\{size\}px`/);
assert.match(indexSource, /button\.style\.height = `\$\{size\}px`/);
assert.match(indexSource, /if \(wasTap\) openPanelSafely\(\)/);
assert.match(indexSource, /suppressClickUntil = Date\.now\(\) \+ 700/);
assert.match(indexSource, /Pointer capture unavailable/);
assert.match(indexSource, /打开失败：\$\{detail\}/);
