...
const worldbookSelectionSource = fs.readFileSync(new URL('../worldbook-selection.js', import.meta.url), 'utf8');
...
assert.match(indexSource, /uniqueWorldInfoIds/);
assert.match(indexSource, /migrateWorldEntrySelectionKeys/);
assert.match(indexSource, /displayWorldInfoName/);
...
assert.match(worldbookSelectionSource, /file_id/);
assert.match(worldbookSelectionSource, /transport identities/);
assert.match(worldbookSelectionSource, /worldBookSourceKey\(name\)[\s\S]*String\(name \?\? ''\)/);
assert.match(worldbookSelectionSource, /\\p\{L\}/);
