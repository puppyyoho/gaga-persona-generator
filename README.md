...
- 可选真流式输出：Chat Completion、Text Completion 或连接管理器支持时边生成边显示，固定底栏同步显示已接收字数与有效分片数。
- 流式诊断：区分真实多分片、上游一次性完整分片、接口不支持与请求失败；失败时先展示原因，再自动回退为普通生成。
...
真正的实时输出要求模型服务、API 反向代理、SillyTavern 服务端与浏览器之间都持续转发 SSE 数据。生成时底栏会显示有效分片数；数量持续增加代表浏览器正在实时收到内容。

如果最终提示“上游只返回了一个完整分片”，说明插件发出了流式请求，但模型服务或中间代理直到末尾才交付正文。浏览器无法提前展示尚未收到的数据，建议先用同一设备直接访问 SillyTavern 局域网地址测试，再检查隧道或反向代理是否开启了响应缓冲。
